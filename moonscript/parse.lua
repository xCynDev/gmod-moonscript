...
local build_grammar = wrap_env(debug_grammar, function(root)
  ...
  local g = P({
    ...
    BackslashChainItem = symx("\\") * _Name / mark("colon"),
    ColonChain = (ColonChainItem + BackslashChainItem) * (Invoke * ChainItems ^ -1) ^ -1,
    ...
    KeyValue = ((sym(":") - sym("::")) * -SomeSpace * Name * lpeg.Cp()) / self_assign + Ct((KeyName + sym("[") * Exp * sym("]") + Space * DoubleString + Space * SingleString) * (sym(":") - sym("::")) * (Exp + TableBlock + SpaceBreak ^ 1 * Exp)),
